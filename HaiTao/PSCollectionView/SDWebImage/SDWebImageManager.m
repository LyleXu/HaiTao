...
- (void)imageCache:(SDImageCache *)imageCache didFindImage:(UIImage *)image forKey:(NSString *)key userInfo:(NSDictionary *)info
{
    ...
    if ([delegate respondsToSelector:@selector(webImageManager:didFinishWithImage:forURL:)])
    {
        ...
        ((void(*)(id, SEL, id, UIImage *,NSURL*))objc_msgSend)(delegate, @selector(webImageManager:didFinishWithImage:forURL:), self, image, url);
    }
    if ([delegate respondsToSelector:@selector(webImageManager:didFinishWithImage:forURL:userInfo:)])
    {
        ...
        //objc_msgSend(delegate, @selector(webImageManager:didFinishWithImage:forURL:userInfo:), self, image, url, userInfo);
          ((void(*)(id, SEL, id, UIImage *,NSURL*,NSDictionary*))objc_msgSend)(delegate, @selector(webImageManager:didFinishWithImage:forURL:), self, image, url,userInfo);
    }
#if NS_BLOCKS_AVAILABLE
    ...
    for (NSInteger idx = (NSInteger)[downloaders count] - 1; idx >= 0; idx--)
    {
        ...
        if (aDownloader == downloader)
        {
            ...
            if ([delegate respondsToSelector:@selector(webImageManager:didProgressWithPartialImage:forURL:)])
            {
                ...
                ((void(*)(id, SEL, id, UIImage *,NSURL*))objc_msgSend)(delegate, @selector(webImageManager:didProgressWithPartialImage:forURL:), self, image, downloader.url);
            }
            if ([delegate respondsToSelector:@selector(webImageManager:didProgressWithPartialImage:forURL:userInfo:)])
            {
                ...
                ((void(*)(id, SEL, id, UIImage *,NSURL*,NSDictionary*))objc_msgSend)(delegate, @selector(webImageManager:didProgressWithPartialImage:forURL:), self, image, downloader.url,userInfo);
            }
        }
    }
}
        ...
        if (aDownloader == downloader)
        {
            ...
            if (image)
            {
                ...
                if ([delegate respondsToSelector:@selector(webImageManager:didFinishWithImage:forURL:)])
                {
                    ...
                    ((void(*)(id, SEL, id, UIImage *,NSURL*))objc_msgSend)(delegate, @selector(webImageManager:didFinishWithImage:forURL:), self, image, downloader.url);
                }
                if ([delegate respondsToSelector:@selector(webImageManager:didFinishWithImage:forURL:userInfo:)])
                {
                    NSDictionary *userInfo = [downloader.userInfo objectForKey:@"userInfo"];
                   ...
                   // objc_msgSend(delegate, @selector(webImageManager:didFinishWithImage:forURL:userInfo:), self, image, downloader.url, userInfo);
                     ((void(*)(id, SEL, id, UIImage *,NSURL*,NSDictionary*))objc_msgSend)(delegate, @selector(webImageManager:didFinishWithImage:forURL:), self, image, downloader.url,userInfo);
                }
#if NS_BLOCKS_AVAILABLE
            ...
            else
            {
                ...
                if ([delegate respondsToSelector:@selector(webImageManager:didFailWithError:forURL:)])
                {
                    ...
                    ((void(*)(id, SEL, id, UIImage *,NSURL*))objc_msgSend)(delegate, @selector(webImageManager:didFailWithError:forURL:), self, nil, downloader.url);
                }
                if ([delegate respondsToSelector:@selector(webImageManager:didFailWithError:forURL:userInfo:)])
                {
                    ...
                    ((void(*)(id, SEL, id, UIImage *,NSURL*,NSDictionary*))objc_msgSend)(delegate, @selector(webImageManager:didFailWithError:forURL:), self, nil, downloader.url,userInfo);
                }
#if NS_BLOCKS_AVAILABLE
        ...
        if (aDownloader == downloader)
        {
            ...
            if ([delegate respondsToSelector:@selector(webImageManager:didFailWithError:forURL:)])
            {
                //objc_msgSend(delegate, @selector(webImageManager:didFailWithError:forURL:), self, error, downloader.url);
                 ((void(*)(id, SEL, id, NSError *,NSURL*))objc_msgSend)(delegate, @selector(webImageManager:didFailWithError:forURL:), self, error, downloader.url);
            }
            if ([delegate respondsToSelector:@selector(webImageManager:didFailWithError:forURL:userInfo:)])
            {
                NSDictionary *userInfo = [downloader.userInfo objectForKey:@"userInfo"];
                ...
                ((void(*)(id, SEL, id, NSError *,NSURL*,NSDictionary*))objc_msgSend)(delegate, @selector(webImageManager:didFailWithError:forURL:), self, error, downloader.url,userInfo);
            }
#if NS_BLOCKS_AVAILABLE
            if ([downloader.userInfo objectForKey:@"failure"])
            {
